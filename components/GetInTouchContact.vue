  ...
  <section ref="sectionRef" class="relative bg-[#331405] py-16 sm:py-20 lg:py-24 overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl xl:max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="text-center mb-16 lg:mb-20">
        ...
        <p class="contact-page-header-reveal mt-4 sm:mt-5 text-white/80 text-sm sm:text-base leading-relaxed max-w-xl mx-auto whitespace-pre-line">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-12 md:gap-y-16">
        ...
        >
          ...
          <div class="contact-page-icon-reveal w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[#df9c4d] mb-5 sm:mb-6 transition-transform duration-500 hover:-translate-y-1 hover:bg-white/10 cursor-default">
            <div v-html="item.svg"></div>
          </div>

          <!-- Item Title -->
          <h3 class="contact-page-text-reveal font-sans font-semibold text-[#df9c4d] text-sm uppercase tracking-widest mb-3">
            {{ item.title }}
          </h3>

          <!-- Item Details -->
          <p class="contact-page-text-reveal font-sans text-white/90 text-sm leading-relaxed whitespace-pre-line" v-html="item.details">
          </p>
...
const initScrollAnimation = () => {
  ...
  ctx = gsap.context(() => {
    ...
    // 1. Header texts
    tl.from('.contact-page-header-reveal', { opacity: 0, y: 20, duration: 0.8, stagger: 0.1, ease: 'power3.out' })
    // 2. Icons pop in
    .from('.contact-page-icon-reveal', { opacity: 0, y: 20, duration: 0.6, stagger: 0.1, ease: 'power3.out' }, "-=0.4")
    // 3. Info texts slide up
    .from('.contact-page-text-reveal', { opacity: 0, y: 20, duration: 0.6, stagger: 0.1, ease: 'power3.out' }, "-=0.4")
